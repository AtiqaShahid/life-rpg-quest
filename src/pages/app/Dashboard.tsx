...
export default function Dashboard() {
  ...
  const dailyQuests = p.quests.filter((q) => {
    if (!q.is_daily) return false;
    const status = (q as unknown as { status?: string }).status;
    // Only show quests the user can actually act on (or already completed today).
    return !status || status === "active" || status === "locked" || status === "completed";
  });
