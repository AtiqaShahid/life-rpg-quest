...
export const StatBadge = ({ stat, value }: { stat: StatKey; value: number }) => {
  ...
  return (
    <div className="glass group relative flex flex-col items-center gap-1.5 rounded-xl p-3 transition-all hover:scale-[1.03]" style={{ borderColor: `hsl(${meta.colorVar} / 0.25)` }}>
      <div
        className="flex h-9 w-9 items-center justify-center rounded-lg ring-1 transition-all group-hover:scale-110"
        style={{
          background: `hsl(${meta.colorVar} / 0.12)`,
          color: `hsl(${meta.colorVar})`,
          boxShadow: `0 2px 8px -2px hsl(${meta.colorVar} / 0.25)`,
        }}
      >
