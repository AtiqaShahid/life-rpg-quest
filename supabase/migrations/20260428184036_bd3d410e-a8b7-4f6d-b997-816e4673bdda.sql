...
BEGIN
  ...
  IF v_acts_prev7 > 0 AND v_acts_recent7 < v_acts_prev7 THEN
    v_trends := v_trends || to_jsonb(
      ...
      '% week-over-week (' || v_acts_prev7 || ' → ' || v_acts_recent7 || ').'
    );
  ...
  IF v_completion_prev > 0 AND v_completion_recent < v_completion_prev - 10 THEN
    v_trends := v_trends || to_jsonb(
      ...
      ' pts vs prior week (' || ROUND(v_completion_prev) || '% → ' || ROUND(v_completion_recent) || '%).'
    );
  END IF;

  IF v_max_gap >= 2 THEN
    v_trends := v_trends || to_jsonb(
      'Longest inactivity gap in last 14d: ' || v_max_gap || ' consecutive days.'
    );
  END IF;
  ...
  IF v_hard_acts >= 5 AND v_xp_prev > 0 AND v_xp_recent < v_xp_prev * 0.7 THEN
    v_trends := v_trends || to_jsonb(
      ...
      ROUND((1 - v_xp_recent::numeric / v_xp_prev) * 100) || '% XP drop — burnout signature.'
    );
  END IF;

  IF v_hour_stddev IS NOT NULL AND v_hour_stddev > 4 THEN
    v_trends := v_trends || to_jsonb(
      'Activity timing erratic (hour σ=' || ROUND(v_hour_stddev, 1) || ') — energy windows unstable.'
    );
  END IF;
  ...
  IF v_streak > 0 AND v_inactive_days >= 1 THEN
    v_predictions := v_predictions || to_jsonb(
      ...
      GREATEST(1, 2 - v_inactive_days) || ' day(s) without activity.'
    );
  END IF;

  IF v_consistency < 50 AND v_acts_recent7 < v_acts_prev7 THEN
    v_predictions := v_predictions || to_jsonb(
      'At current decline, streak break probable in 3–4 days.'
    );
  END IF;
  ...
  IF v_hard_acts >= 5 AND v_completion_recent < 50 THEN
    v_predictions := v_predictions || to_jsonb(
      ...
      ROUND(v_completion_recent) || '%.'
    );
  END IF;

  IF v_consistency >= 70 AND v_discipline >= 60 AND v_acts_recent7 >= v_acts_prev7 THEN
    v_predictions := v_predictions || to_jsonb(
      'Trajectory: level-up likely within ~1 week if pace holds.'
    );
  END IF;

  IF v_inactive_days >= 3 THEN
    v_predictions := v_predictions || to_jsonb(
      'Inactive ' || v_inactive_days || ' days — re-entry friction grows after day 5.'
    );
  END IF;

  IF v_hard_acts >= 5 AND v_completion_recent < 50 THEN
    v_recommendations := v_recommendations || to_jsonb(
      'Reduce task difficulty by ~20% for the next 3 days to recover throughput.'
    );
  END IF;

  IF v_consistency < 50 THEN
    v_recommendations := v_recommendations || to_jsonb(
      'Switch to shorter (≤15 min) tasks to rebuild streak momentum.'
    );
  END IF;

  IF v_consistency >= 70 AND v_discipline >= 60 THEN
    v_recommendations := v_recommendations || to_jsonb(
      'Increase challenge ~10%: add one hard-difficulty task to your daily set.'
    );
  END IF;

  IF v_hour_stddev IS NOT NULL AND v_hour_stddev > 4 THEN
    v_recommendations := v_recommendations || to_jsonb(
      'Anchor a fixed daily window (±1h) — stable timing compounds output.'
    );
  END IF;

  IF v_inactive_days >= 2 THEN
    v_recommendations := v_recommendations || to_jsonb(
      'Log one easy activity today to interrupt the inactivity slope.'
    );
  END IF;
...
GRANT EXECUTE ON FUNCTION public.get_life_score() TO authenticated;